...
// Wider than the generator's 16rem to fit the longest label the admin nav
// actually carries, "Interview experiences", on one line at the larger type
// size set on SidebarMenuButton below.
...
const sidebarMenuButtonVariants = cva(
  // Two deliberate departures from the generated file, both applying to every
  // sidebar in the portal so the two shells cannot drift:
  //
...
function SidebarMenuSkeleton({
  ...
  // A width between 50% and 90%, varied per row so a column of these reads as
  // lines of text rather than a bar chart.
  //
  // The generated file picks it with `Math.random()`, which is not just the
  // impurity the lint rule names: the server and the client roll different
  // numbers, so every skeleton row is a hydration mismatch. `useId` is stable
  // across both and already unique per instance, so hashing it keeps the
  // variety and removes the mismatch.
