...
export async function saveJobProfile(formData: FormData): Promise<JobProfileActionResult> {
  ...
  const parsed = jobProfileFormSchema.safeParse({
    ...
    min10Percent: formData.get("min10Percent"),
    min12Percent: formData.get("min12Percent"),
  ...
  const data = {
    ...
    min10Percent: parsed.data.min10Percent,
    min12Percent: parsed.data.min12Percent,
