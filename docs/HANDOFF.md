...
- Last verified (2026-09-23, performance pass — indexes, SQL search, analytics cache): `npm run lint`, `npm run type-check` (one pre-existing, unrelated failure — `frontend/src/lib/redis-client.ts` imports `ioredis`, which is not installed anywhere in this checkout; untouched by this pass, confirmed by `git diff --stat` showing no change to that file), `npm test` (152 passed), and `make test-backend` (233 passed, up from 231 — two new files, `test_applications_admin_list.py` and `test_analytics_cache.py`). Scope was agreed with the user up front as a deliberately safe subset of a longer performance review, explicitly excluding a rework of `DataTable`'s fetch contract (used by ~12 other admin tables) as its own separately-scoped item — see the 2026-09-23 DECISIONS entry for the full reasoning and every file touched. Verified live against the running dev stack: created the Prisma migration `20260922194813_add_application_jobprofile_indexes` with `prisma migrate dev --create-only` (a bare `migrate dev` hung indefinitely on `pg_advisory_lock` from a stray prior `prisma migrate dev` process that had to be killed first — worth knowing if this happens again), applied it with `prisma migrate deploy`, ran `prisma generate`, and confirmed all four indexes present with `docker compose exec db psql ... -c '\d "Application"'` / `'\d "JobProfile"'`. Rebuilt the `backend` Docker image and ran the full pytest suite in-container after every source change (the image is not volume-mounted, so a host-side edit needs a rebuild before `docker compose run --rm backend pytest` sees it — cost about 20 minutes of this pass chasing a stale-image false test failure before that was confirmed). Brought the full stack back up with `docker compose up -d` afterward and confirmed the backend started healthy with `docker compose logs backend` and `GET /api/health` → 200. **Not clicked in a signed-in browser**, the standing reason repeated throughout this file: nobody on this machine has an admin password. What that leaves unchecked is the actual page-load latency improvement and whether the admin applications/students pages still render correctly with the bounded queries in front of them (they should — the response shape is unchanged, only the row count returned is now bounded, and the new backend tests inspect the exact SQL statement each endpoint builds rather than trusting a mock's canned return value).
- Last verified (2026-09-20, rich text announcements pass): `npm run lint`, `npm run type-check`, `npm run build`, 135 frontend unit tests (5 new, over `sanitizeAnnouncementHtml`/`stripHtmlToText`), and 200 backend pytest tests (3 new, over the Pydantic validators). The announcement body switched from a plain `<textarea>` to a shadcn-styled Tiptap editor (`components/ui/rich-text-editor.tsx`) in three places: the composer (`announcement-composer.tsx`), the admin edit modal (`announcements-manager.tsx`, where the editor is controlled React state mirrored into a hidden `<input name="content">` since the modal still submits through a native form action), and the two read paths that display it (`announcements-manager.tsx`'s preview modal, `dashboard-feed.tsx`'s list and detail views). `Announcement.content` is now sanitized HTML rather than plain text — see the 2026-09-20 DECISIONS entry for the allow-list and where each sanitize/strip call lives. Rebuilt the `backend` Docker image twice (once for the new `bleach` dependency, once after fixing a test assertion) and ran the full suite in-container both times; the `frontend` Docker image was also rebuilt after the production build succeeded locally. **Not clicked in a signed-in browser** — same standing reason as every other admin-screen entry below: nobody on this machine has an admin password. What that leaves unchecked is the toolbar's actual click behaviour (bold/italic/link popover/undo-redo) and the rendered HTML's visual layout; the sanitization boundary itself (what tags survive, what a hand-built request smuggling `<script>`/`onerror`/`javascript:` gets stripped to) is covered by the new unit and pytest tests, and the composer/editor code type-checks and builds.
