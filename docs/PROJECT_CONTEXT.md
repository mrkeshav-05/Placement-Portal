...
- Job eligibility is evaluated from the student's current profile and job criteria: CGPA, 10th/12th percentage, batch, branch, degree, gender, backlogs, placement bans, and document completeness. An empty `allowedDegrees` or `allowedGenders` list, or one holding `all`/`any`, places no restriction; a null `min10Percent`/`min12Percent` places no restriction the same way; a restriction the profile cannot answer fails.
...
- There are five roles: `STUDENT` (Tier 1), `FACULTY` (Tier 2), `PLACEMENT_VOLUNTEER` (Tier 3), `PLACEMENT_TEAM` (Tier 4), `SUPER_ADMIN` (Tier 5), backed by a 49-entry `module.action` permission catalog. A `_own` suffix (`applications.view_own`) means the holder reaches only rows they own; the route still applies the ownership filter. `FACULTY` is deliberately narrow and entirely read-only: `analytics.view`, `noc.view`, and `placement_records.view`, nothing else — see the 2026-09-21 entry in `docs/DECISIONS.md`. It is also the one role excluded from `isElevatedRole`/`is_elevated_role`'s broader "trust this role for anything not explicitly permission-gated" shortcut, on purpose.
